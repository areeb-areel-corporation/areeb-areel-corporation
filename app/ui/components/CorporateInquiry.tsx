...
export default function CorporateInquiry() {
  const [formData, setFormData] = useState({ name: '', email: '', division: 'General Corporate', message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus('success');
        setFormData({ name: '', email: '', division: 'General Corporate', message: '' });
          ...
          >
            <div>
              ...
              <p className="text-brand-silver/70 text-lg leading-relaxed max-w-md">
                Whether you are seeking strategic partnerships, premium real estate acquisitions, or franchise opportunities within our energy network, our executive team is ready to assist you.
              </p>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-white font-bold tracking-wide">Global Headquarters</h4>
                  <p className="text-brand-silver/60 text-sm mt-1">Areeb & Areel Tower, Corporate District<br />Lahore, Pakistan</p>
                </div>
              </div>
            </div>
          </motion.div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <motion.div variants={itemVariants as any} className="relative group">
                    ...
                    />
                    <label htmlFor="name" className="absolute left-0 top-4 text-brand-silver/50 text-sm transition-all peer-focus:-top-3 peer-focus:text-xs peer-focus:text-[#D4AF37] peer-valid:-top-3 peer-valid:text-xs peer-valid:text-brand-silver/80">
                      Full Name
                    </label>
                  </motion.div>
                  ...
                  <motion.div variants={itemVariants as any} className="relative group">
                    ...
                    />
                    <label htmlFor="email" className="absolute left-0 top-4 text-brand-silver/50 text-sm transition-all peer-focus:-top-3 peer-focus:text-xs peer-focus:text-[#D4AF37] peer-valid:-top-3 peer-valid:text-xs peer-valid:text-brand-silver/80">
                      Corporate Email
                    </label>
                  </motion.div>
                </div>
                ...
                <motion.div variants={itemVariants as any} className="relative group">
                  ...
                  >
                    <option value="General Corporate" className="bg-[#111111] text-white">General Corporate Inquiry</option>
                    <option value="AAA Developments" className="bg-[#111111] text-white">AAA Developments (Real Estate)</option>
                    <option value="Total Parco Hubs" className="bg-[#111111] text-white">Energy Hubs & Petrol Pumps</option>
                    <option value="Express Smart Mart" className="bg-[#111111] text-white">Retail & Smart Marts</option>
                  </select>
                  <label className="absolute left-0 -top-3 text-brand-silver/80 text-xs">Target Division</label>
                </motion.div>
                ...
                <motion.div variants={itemVariants as any} className="relative group">
                  ...
                  />
                  <label htmlFor="message" className="absolute left-0 top-4 text-brand-silver/50 text-sm transition-all peer-focus:-top-3 peer-focus:text-xs peer-focus:text-[#D4AF37] peer-valid:-top-3 peer-valid:text-xs peer-valid:text-brand-silver/80">
                ...
                <motion.div variants={itemVariants as any} className="pt-4 flex items-center justify-between">
                  ...
                  >
                    <span className="relative z-10 flex items-center gap-2">
                      {status === 'loading' ? 'Transmitting...' : status === 'success' ? 'Received' : 'Submit Inquiry'}
                  ...
                  {/* Error Message */}
                  {status === 'error' && (
                    <p className="text-red-400 text-sm font-medium">Transmission failed. Please try again.</p>
                  )}
                </motion.div>
